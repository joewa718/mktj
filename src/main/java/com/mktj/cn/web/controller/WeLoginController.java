...
public class WeLoginController extends WxMpUserQuery {
    ...
    public void login(HttpServletRequest request, HttpServletResponse response, @RequestParam(value = "state", required = false) String state) throws IOException {
        GenerateRandomCode generateRandomCode = new GenerateRandomCode();
        String connectUrl = wxService.oauth2buildAuthorizationUrl(LOGIN_CALLBACK, "snsapi_userinfo", StringUtils.isBlank(state) ? generateRandomCode.generate(5) : state);
